...
gameIcon = pygame.image.load('gameico.png')
...
class Player():
    ...
    def reset(this):
        this.length = 1 #go back to being a 1 square snake
        this.positions = [((screen_width/2), (screen_height/2))]
        this.direction = random.choice([up, down, left, right])
        this.score = 0 #initial score on reset is zero 
...
def main(): #starts all the functions , main function
    ...
    while (1): #while loop makes it possible to make game run continuesly until an action is taken by the user
